...
while True:
    capital = input("please enter a capital: ").lower()
...
while game == 1:
    print(f"correct answer {capital.title()} is the capital of {country_capitals[capital].title()}")
    last_letter = capital[-1]
    capital = input(f"what is the capital of the world starts with '{last_letter.upper()}' letter?").lower()
    if capital[0] == last_letter and capital in country_capitals:
        score += 1
    else:
        game = 0
        for letter in country_capitals:
            if letter[0] == last_letter:
                print(f"wrong, '{capital}' is not correct, you can say {letter.title()} the capital of country {country_capitals[letter].title()}")
                break

print(f"your score is {score}")
# my capital game project